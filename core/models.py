...
class Item(models.Model):
    ...
    item_number = models.CharField(unique = True, max_length=10, default="")
...
# def get_upload_path(a,  b):
#     print("AA",a, b)
#     item_name = "demo"
#     return f'media/{item_name}/'

def get_file_name(instance, filename):
    total = instance.get_total_items()
    print(total)
    ext = filename.split('.')[-1]
    filename = "%s_%s_%s.%s" % ( instance.item_slug, instance.item_slug.item_name, total, ext)
    # print("instance",instance.item_slug.item_number)
    model = instance.item_slug.item_number
    print(model)
    name = str(model).replace(' ', '_')
    return f'multiple_images_of_items/{name}/images/{filename}'


class ItemMultipleImages(models.Model):
    item_slug = models.ForeignKey(Item, on_delete=models.CASCADE)
    multipleimages = models.FileField(upload_to=get_file_name, blank=True,null=True)

    def get_total_items(self):
        return len(ItemMultipleImages.objects.all() )
